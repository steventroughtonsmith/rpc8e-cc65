...
package: library padimage
	tar --transform "s,^,cc65-rpc8_$$(git rev-list --max-count=1 HEAD)/,S" -cjf cc65-rpc8_$$(git rev-list --max-count=1 HEAD).tar.bz2 util/padimage.c padimage rpc8e.cfg rpc8e.lib
...
testsuite: test
	...
	cl65 --cpu 65816 -Osir -g -T -t none -c -o test_malloc.o test/test_malloc.c
	ld65 -o test_malloc.img --define __STACKSIZE__="$$200" -C rpc8e.cfg -m test_malloc.map  test_malloc.o rpc8e.lib
	rm test_malloc.o
	./padimage test_malloc.img
